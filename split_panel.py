...
class SplitPanel(wx.Panel):
    ...
    def __init__(self, parent):
        ...
        self.pdf_split_options = wx.TextCtrl(self, validator=CharValidator("no-alpha"))
        row_sizer.Add(self.pdf_split_options, 0, wx.ALL, 5)
        main_sizer.Add(row_sizer)

        msg = (
            "Type page numbers and/or page ranges separated by commas."
            " For example: 1, 3 or 4-10. Note you cannot use both commas "
            "and dashes."
        )
        directions_txt = wx.TextCtrl(
            self, value=msg, style=wx.TE_MULTILINE | wx.NO_BORDER
        )
    ...
    def split(self, input_pdf, output_path, split_options):
        ...
        self.show_message(message, caption="Split Finished", style=wx.ICON_INFORMATION)

    def show_message(self, message, caption="Error", style=wx.ICON_ERROR):
        with wx.MessageDialog(
            None, message=message, caption=caption, style=style
        ) as dlg:
            dlg.ShowModal()
